...
try {
    const unchangedFu = ( currentIp ) => {
        console.log( `${ dayjs().format( "YYYY-MM-DD HH:mm:ss" ) } ip未变动：${ currentIp }` )
    }

    const changeFu = async ( currentIp ) => {
        console.log( `${ dayjs().format( "YYYY-MM-DD HH:mm:ss" ) } ip变动：${ currentIp }` )
        ...
        if ( configFile.appConfig?.length, configFile ) {
            configFile.appConfig?.map( async ( item ) => {
                const data = await setApi( configFile.cookie, item.app_id, [ currentIp ] )
